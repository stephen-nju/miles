"""GLM-4.7 Full (355B-A32B) fully-async agentic training with SWE-bench data.
...
Same model architecture as GLM-4.5-355B-A32B. Default split: 4 nodes training
+ 12 nodes inference (configurable via --train-num-nodes).
...
Usage:
    python run-glm47-agentic-async.py --num-nodes 16
    python run-glm47-agentic-async.py --num-nodes 32 --train-num-nodes 16
    python run-glm47-agentic-async.py --num-nodes 16 --rollout-fp8
    python run-glm47-agentic-async.py --num-nodes 16 \\
        --agent-server-url http://ts-egress-agent-server:8080
"""
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    save_dir: str = "/root/GLM-4.7-Full_agentic_async/"
    ...
    agent_server_url: str = os.environ.get(
        "AGENT_SERVER_URL", "http://ts-egress-agent-server:8080"
    )
    ...
    prometheus_run_name: str = "glm47-full-swe-async"
...
def execute(args: ScriptArgs):
    ...
    # SGLang: multi-node engines with EP + DP-attention on dedicated rollout nodes.
