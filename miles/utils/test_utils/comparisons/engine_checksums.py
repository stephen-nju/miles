...
def _checksums_by_rollout_id(events: list[EngineWeightChecksumEvent]) -> dict[int, dict[str, str]]:
    by_rollout: dict[int, dict[str, str]] = {}
    for event in events:
        assert event.rollout_id not in by_rollout, f"Duplicate EngineWeightChecksumEvent for rollout {event.rollout_id}"
